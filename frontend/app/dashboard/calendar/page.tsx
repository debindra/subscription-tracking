...
export default function CalendarPage() {
  ...
  const itemsByDay = useMemo(() => {
    const map = new Map<string, { type: 'renewal' | 'trial'; sub: Subscription }[]>();
    for (const s of subs) {
              ...
              <div className="space-y-1">
                {items.slice(0, 3).map((entry, idx) => (
                  <div
                    ...
                    title={`${entry.type === 'trial' ? 'Trial ends: ' : 'Renewal: '} ${entry.sub.name} • ${formatDate(
                      entry.type === 'trial' ? (entry.sub.trialEndDate as string) : entry.sub.nextRenewalDate
                    )}`}
                  >
